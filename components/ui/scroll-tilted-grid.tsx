...
function Tile({
  ...
  const contrast = useTransform(p, [0, 0.5, 1], [4, 1, 4], {
    ease: focusEase,
  });
...
export type ScrollTiltedGridProps = {
  ...
  columns?: 1 | 2;
  /** Tile tilt side when `columns` is 1. Default `"L"`. */
  side?: "L" | "R" | "alternate";
...
export function ScrollTiltedGrid({
  ...
  const resolveSide = (index: number): Side => {
    if (columns === 2) return index % 2 === 0 ? "L" : "R";
    if (side === "alternate") return index % 2 === 0 ? "L" : "R";
    return side;
  };

  const gridClass = [
    "mx-auto mt-[20vh] mb-[10vh] grid w-full px-6 py-[20vh]",
